...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    for (int x = min_x; x < max_x; x++) {
        for (int y = min_y; y < max_y; y++) {
            ...
            // if without this bool, when rasterize no-first triangle, 
            // frame_buf_SSAA in bounding box and not in triangle 
            // while be set {0,0,0} no matter weather it already has color or not.
            bool isInTriangle = false;
            for (int x_SSAA = 0; x_SSAA < SSAA_MUL; x_SSAA++) {
                for (int y_SSAA = 0; y_SSAA < SSAA_MUL; y_SSAA++) {
                    
                    float real_x = x + x_SSAA / SSAA_MUL + 1 / (2 * SSAA_MUL), 
                        real_y = y + y_SSAA / SSAA_MUL + 1 / (2 * SSAA_MUL);
                    if (insideTriangle(real_x, real_y, t.v)) {
                        ...
                        if (z_interpolated < depth_buf_SSAA[get_index_SSAA(x, y, x_SSAA, y_SSAA)]) {
                            Eigen::Vector3f point(x, y, 1.0f);
                            set_pixel_SSAA(point, x_SSAA, y_SSAA, t.getColor());
                            depth_buf_SSAA[get_index_SSAA(x, y, x_SSAA, y_SSAA)] = z_interpolated;
                        }
                        color += frame_buf_SSAA[get_index_SSAA(x, y, x_SSAA, y_SSAA)];
                    }
                }
            }
            // downsampling
            if (isInTriangle) {
                Eigen::Vector3f point(x, y, 1.0f);
                set_pixel(point, color / (SSAA_MUL * SSAA_MUL));
            }
        }
    }
    return;
